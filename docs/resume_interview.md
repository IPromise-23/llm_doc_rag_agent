...
- 基于 LangGraph 实现 `source_lookup/direct_answer/retrieve_rag` 自适应路由，并加入规则型 CRAG/Self-RAG trace，支持检索相关性判断、query rewrite 和答案 groundedness 记录。
- 编写单元测试覆盖 loader、chunking、retrieval、service、graph routing、rewrite retry 与 insufficient context 分支，当前核心测试 `27 passed`。
...
- 实现轻量 CRAG/Self-RAG 控制策略：检索上下文不足时自动改写 query 并重试，超过预算后返回 insufficient context，降低无依据强答风险。

### 版本 C：更短的一栏版

**llm_doc_rag_agent：本地技术文档 RAG Agent**  
基于 Python、Qdrant、SentenceTransformer、LangGraph 实现本地文档 RAG Agent，支持增量索引、dense/BM25/hybrid 检索、多策略 eval、source lookup、规则型 CRAG/Self-RAG trace 和 CLI/API 入口；通过单元测试覆盖核心检索、路由、query rewrite 与 insufficient context 分支。

## 30 秒自我介绍版

我做了一个本地技术文档 RAG Agent，主要是把之前学习的 RAG、Qdrant 检索评估和 LangGraph 编排，从 notebook 改造成一个模块化工程。它支持本地文档索引、Qdrant 持久化、dense/BM25/hybrid 检索、LangGraph 自适应路由，以及规则型 CRAG/Self-RAG 自检。这个项目的重点不是做一个聊天 demo，而是把 RAG Agent 的索引、检索、路由、trace、eval 和测试都串成一个能复盘的工程实践。
...
### 4. CRAG/Self-RAG 为什么先做规则型？

主要是成本和可测试性。企业级方案可以用 LLM judge 判断文档相关性和答案 groundedness，但这个项目定位是实习实践项目。如果每次测试都依赖外部 LLM，成本、稳定性和可复现性都会变差。所以我先做规则型 gate：根据 query terms、retrieval score 和重叠比例做轻量判断，保证控制流完整、测试稳定，后续可以替换成 LLM judge。
...
首先 prompt 要求只基于上下文回答。其次 graph 里有 `grade_documents`，如果检索结果为空或低相关，会先 rewrite query 重试。超过 rewrite 预算仍没有足够上下文时，返回 insufficient context，而不是继续调用 LLM 强答。生成之后还会记录 groundedness/relevance trace，方便调试答案是否被上下文支持。

### 7. 项目最大的工程难点是什么？

最大难点是把 notebook 中线性的学习代码改造成模块化工程：需要明确 loader、splitter、embedding、vector store、retriever、agent graph、service、CLI/eval 的边界。另一个难点是控制 Agent 不要无限循环，所以 query rewrite 有 `max_rewrites`，source lookup 也单独分支，避免不必要的 LLM 调用。

### 8. 如果继续优化，你会做什么？

优先做三件事：第一，把 BM25 从临时 payload 计算升级成 Qdrant sparse/named vector 的持久化混合检索；第二，把规则型 grader 替换或增强为 LLM-as-judge，并把 judge prompt 和结果记录进 trace；第三，补一个轻量 UI 或 API streaming，让用户能看到答案、citations、retrieved chunks 和 graph path。
...
- 不要说“CRAG/Self-RAG 已经完全实现论文方案”；更准确说“实现了规则型轻量自检和控制流”。
- 不要说“评估指标已经企业级完整”；更准确说“支持多检索策略对比、报告输出和离线 RAGAS 指标评估，运行时 gate 仍保留可测试的轻量规则实现”。
